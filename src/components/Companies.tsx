  ...
  return (
    <section id="companies" className="py-16 bg-white dark:bg-neutral-950 overflow-hidden border-y border-neutral-100 dark:border-neutral-900">
      ...
      <div className="relative w-full overflow-hidden">
        <motion.div
          className="flex space-x-16 min-w-max will-change-transform"
          animate={{ x: [0, -1000] }}
          transition={{
            x: {
              repeat: Infinity,
            ...
            <div key={i} className="flex items-center justify-center space-x-3 group cursor-pointer">
              <div className="relative h-12 w-auto flex items-center justify-center">
                <img 
                  src={company.logo} 
                  alt={company.name} 
                  className="h-full w-auto object-contain opacity-70 transition-all duration-300 group-hover:opacity-100 group-hover:scale-110 will-change-transform"
                  loading="lazy"
                  decoding="async"
                />
              </div>
